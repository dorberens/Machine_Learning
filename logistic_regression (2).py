...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        w_0 = np.random.randn(X.shape[1]) / np.sqrt(X.shape[1])
        if self.penalty_ == "l1":
            self.coefs_ = self.solver_.fit(
                RegularizedModule(LogisticModule(w_0), L1(w_0), self.lam_, w_0, self.include_intercept_), X, y)
        elif self.penalty_ == "l2":
            self.coefs_ = self.solver_.fit(
                RegularizedModule(LogisticModule(w_0), L2(w_0), self.lam_, w_0, self.include_intercept_), X, y)
        else:
            self.coefs_ = self.solver_.fit(LogisticModule(w_0), X, y)
